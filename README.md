...
My university, Politecnico di Torino, provides useful video lectures visible on the online platform. However, it is not possible to download them all together and watch them offline. You need to download each one after the other and hence I have developed this program to download both course material (e.g. slides) and video lectures directly.
The trickiest thing was that I could not use simple html/xlm parsing tools as BeautifulSoap because of the various amount of JavaScript snippets to generate page content. I needed to use the Selenium WebDriver to interact dynamically with the online platform and donwload then the needed material.
